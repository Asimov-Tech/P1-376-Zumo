...
{//angle is the number of degrees the robot is supposed to turn. 
//Positive numbers are counterclockwise and negative numbers are clockwise. The function ranges from -180 to 179
  
  bool done = false; //incdicates whether the function is over
  int f = 1; //factor for rotation direction
  if(angle<0) f = -1;
  ...
  while(done==false)
  {
    ...
    {//case if function is ongoing
      motors.setSpeeds(f*-100, f*100);
      imustart();
    }
  }
  //resets the turn sensor
  turnSensorReset();
  turnAngleDegrees = 0;
}
...
//Updates the turn sensor and print the angle on the LCD. Good for debugging
void imustart()
{
  turnSensorUpdate();
